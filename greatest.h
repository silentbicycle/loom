...
extern greatest_type_info greatest_type_info_fmt;
...
#define GREATEST_ASSERT_EQ_FMT(FMT, EXP, GOT)                           \
    do {                                                                \
        const char _fmt[] = FMT;                                        \
        GREATEST_ASSERT_EQUAL_Tm("", (void *)EXP, (void *)GOT,          \
            &greatest_type_info_fmt, (void *)_fmt);                     \
    } while (0)                                                         \
...
greatest_type_info greatest_type_info_string = {                        \
    ...
    greatest_string_printf_cb,                                          \
                                                                        ...
                                                                        \
static int greatest_fmt_equal_cb(const void *exp, const void *got,      \
    void *udata) {                                                      \
    (void)udata;                                                        \
    long long lle = (long long )exp;                                    \
    long long llg = (long long )got;                                    \
    return lle < llg ? -1 : lle > llg ? 1 : 0;                          \
}                                                                       \
                                                                        \
static int greatest_fmt_printf_cb(const void *t, void *udata) {         \
    const char *fmt = (const char *)udata;                              \
    long long v = (long long)t;                                         \
    return fprintf(GREATEST_STDOUT, fmt, v);                            \
}                                                                       \
                                                                        \
greatest_type_info greatest_type_info_fmt = {                           \
    greatest_fmt_equal_cb,                                              \
    greatest_fmt_printf_cb,                                             \
};                                                                      \
                                                                        \
