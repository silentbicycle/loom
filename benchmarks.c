    ...
    for (size_t i = 0; i < sizeof(benchmarks)/sizeof(benchmarks[0]); i++) {
        ...
        for (i = 0; i < 10 * MAX_SHUTDOWN_SECONDS; i++) {
            ...
            if (i > 0 && cfg.verbosity > 1 && ((i % 10) == 0)) {
                printf(" -- loom_shutdown, %d msec have passed\n", i * 100);
            }
        }
...
static void calc_pi_cb(void *env) {
    pi_env *p = (pi_env *)env;
    if (p->delay >= 0) { poll(NULL, 0, p->delay); }
    double acc = 0;
...
typedef int (bench_delay_cb)(int nth_test);
...
static int no_delay_cb(int nth_test) {
    (void)nth_test;
    return 0;
}
...
static int small_delay_cb(int nth_test) {
